...
F_type1.per.de = rep(NA, nn) # type one error after permutation (direct decision method)

## for smaller sample size
for (kk in 1:split.point){
  
  load(paste0("/Users/erjing/Desktop/result_F/F_type1/F_type1error_",kk,".rda"))
...
for (kk in 1:split.point){
  ...
  load(paste0("/Users/erjing/Desktop/result_F/F_type1.per/F_type1error.per_",kk,".rda"))
...
for (kk in 1:split.point){
  ...
  load(paste0("/Users/erjing/Desktop/result_F/F_type1.per.de/F_type1error.per.de_",kk,".rda"))
  F_type1.per.de[kk] = F_type1error.per.de
...
decision.s = matrix(NA, nexperiment, nn-split.point) 

for (kk in (split.point+1):nn){
  
  for (ss in 1:(nexperiment/split.nexp)){
    
    load(paste0("/Users/erjing/Desktop/result_F/F_p.value/F_p.value_",kk,"_",ss,".rda"))
    load(paste0("/Users/erjing/Desktop/result_F/F_p.value.per/F_p.value.per_",kk,"_",ss,".rda"))
    load(paste0("/Users/erjing/Desktop/result_F/F_decision/F_decision_",kk,"_",ss,".rda"))
    ...
    decision.s[(split.nexp*(ss-1)+1):(split.nexp*ss), (kk-split.point)] = decision
    
  }
  
  F_type1[kk] = mean(p.value.s[, kk-split.point] < alpha)
  F_type1.per[kk] = mean(p.value.per.s[, kk-split.point] < alpha)
  F_type1.per.de[kk] = sum(decision.s[, kk-split.point])/nexperiment
